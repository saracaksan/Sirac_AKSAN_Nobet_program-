...
import io, calendar, json, base64
...
.metric-card:hover { transform: translateY(-2px); box-shadow: 0 8px 32px rgba(15,31,61,.15); }
.metric-card .label { font-size: .78rem; color: var(--muted); font-weight: 600; text-transform: uppercase; letter-spacing: .05em; margin-bottom: 4px; }
...
# ─────────────────────────────────────────────
def get_algo_description(algo_name: str) -> str:
    descs = {
        "1. Sabit Gün – Döngüsel Yer (Önerilen)": (
            "📌 Öğretmenin nöbet günü sabittir (Müsaitlik Matrisi 🟢 günler). "
            "Görev yeri her nöbette döngüsel değişir. Tüm bölgeler eşit kullanılır."
        ),
        "2. Sabit Gün – Sabit Yer": (
            "📌 Hem nöbet günü hem görev yeri değişmez. "
            "Sabit kurallar sekmesinden ayarlanır."
        ),
        "3. Tam Döngüsel (Gün ve Yer Değişir)": (
            "📌 Her hafta hem gün hem yer döner. Müsait olan herkese eşit sırayla verilir. "
            "Maksimum adaleti sağlar."
        ),
        "4. Adalet Puanı ve Dinamik Joker (Yapay Zeka)": (
            "📌 Nöbet sayıları ve raporlu günleri analiz edip eşitliği sağlamaya çalışır. "
            "Boşlukları doldurur."
        ),
    }
...
def metric_card(label: str, value, sub: str = "", color: str = "var(--blue)") -> str:
    return (
...
def get_veya_olustur_duty_setting(db, teacher_id: int) -> TeacherDutySetting:
...
def get_musaitlik(db, school_id: int) -> dict:
...
def set_musaitlik(db, school_id: int, teacher_id: int, gun: int, durum: str):
    key = f"musaitlik_v9_{school_id}"
    if key not in st.session_state:
        st.session_state[key] = {}
    if teacher_id not in st.session_state[key]:
        st.session_state[key][teacher_id] = {}
    st.session_state[key][teacher_id][gun] = durum


def yedek_olustur(db, school_id: int, yil: int, ay: int) -> str:
    ...
    payload = {
        "meta": {
            "school_id": school_id, "yil": yil, "ay": ay,
            "tarih": str(date.today()), "versiyon": "v9.1"
        },
        "ogretmenler": [
            {"id": o.id, "name": o.name_surname, "branch": o.branch,
             "monthly": o.monthly_duty_count, "yearly": o.yearly_duty_count}
            for o in ogretmenler
        ],
        "nobetler": [
            {"date": str(n.date), "type": n.duty_type, "teacher_id": n.teacher_id,
             "location_id": n.location_id, "status": n.status}
            for n in nobetler
        ],
        ...
        "mazeretler":  [{"teacher_id": m.teacher_id, "type": m.leave_type,
                         "start": str(m.start_date), "end": str(m.end_date)} for m in mazeretler],
        "yardimcilar": [{"name": y.name_surname} for y in yardimcilar],
    }
    return json.dumps(payload, ensure_ascii=False, indent=2)


def _temizle_ay_nobetleri(db, school_id: int, yil: int, ay: int, duty_type: str):
    """Belirtilen ay ve nöbet tipindeki tüm nöbetleri sil; sayaçları geri al."""
    bas = date(yil, ay, 1)
    ...
    eski_nobetler = (
        ...
        .filter(
            ...
            DutySchedule.date >= bas,
            DutySchedule.date <= bit,
            DutySchedule.duty_type == duty_type,
        )
        .all()
    )
    for en in eski_nobetler:
        ogr = db.query(User).filter(User.id == en.teacher_id).first()
        if ogr:
            ogr.monthly_duty_count = max(0, (ogr.monthly_duty_count or 0) - 1)
            ogr.yearly_duty_count  = max(0, (ogr.yearly_duty_count or 0) - 1)
...
def akilli_dagitim(
    ...
    algoritma: str = "1. Sabit Gün – Döngüsel Yer (Önerilen)",
) -> tuple[bool, str, str]:
    duty_type = "Haftasonu" if is_weekend else "Ogretmen_Nobeti"

    # Nöbet havuzundaki öğretmenler
    tum_ogretmenler = (
        db.query(User)
        .filter(User.school_id == school_id, User.role == "ogretmen", User.status == "Aktif")
        .all()
    )
    ogretmenler = []
    for ogr in tum_ogretmenler:
        ayar = get_veya_olustur_duty_setting(db, ogr.id)
        if is_weekend and ayar.hafta_sonu_tutar:
            ogretmenler.append(ogr)
        elif not is_weekend and ayar.hafta_ici_tutar:
            ogretmenler.append(ogr)

    # Bölgeler
    bolgeler = (
        db.query(Location)
        .filter(Location.school_id == school_id, Location.location_type == "Hafta Sonu")
        .all()
        if is_weekend
        else db.query(Location)
        .filter(Location.school_id == school_id, Location.location_type != "Hafta Sonu")
        .all()
    )
    ...
    if not ogretmenler:
        return (
            False,
            "❌ İlgili nöbet tipine atanmış aktif öğretmen bulunamadı. "
            "Lütfen Müsaitlik Matrisi sekmesinden Hafta İçi/Sonu tercihlerinizi işaretleyin.",
            "",
        )
    if not bolgeler:
        tip_str = "Hafta Sonu" if is_weekend else "Hafta İçi"
        return False, f"❌ {tip_str} türünde nöbet bölgesi tanımlı değil.", ""

    bas = date(yil, ay, 1)
    bit = date(yil, ay, calendar.monthrange(yil, ay)[1])

    # Eski nöbetleri temizle
    _temizle_ay_nobetleri(db, school_id, yil, ay, duty_type)
    ...
    def siradaki_lokasyonu_bul(t_id: int, available_loc_ids: list) -> int | None:
        son = (
            db.query(DutySchedule)
            .filter(DutySchedule.teacher_id == t_id, DutySchedule.duty_type == duty_type)
    ...
    def gun_gecerli_mi(islem_tarihi: date, hw: int) -> bool:
        if islem_tarihi in oto_tatil or islem_tarihi in man_tatil:
            return False
        if is_weekend and hw < 5:
            return False
        if not is_weekend and hw >= 5:
            return False
        return True

    def izinli_mi(ogr_id: int, islem_tarihi: date) -> bool:
        return any(
            m.teacher_id == ogr_id and m.start_date <= islem_tarihi <= m.end_date
            for m in mazeretler
        )

    def engelli_mi(ogr_id: int, islem_tarihi: date) -> bool:
        return any(dp.teacher_id == ogr_id and dp.blocked_date == islem_tarihi for dp in ozel_kapat)

    # ── FAZ 1: SABİT KURALLAR ──────────────────
    for g in range(1, bit.day + 1):
        islem_tarihi = date(yil, ay, g)
        hw = islem_tarihi.weekday()
        week_num = get_week_number(islem_tarihi)
        if not gun_gecerli_mi(islem_tarihi, hw):
            continue

        atanan_b = []
        gunluk_atanan = []
        for sab in sabitlemeler:
            if sab.day_of_week != hw and sab.day_of_week is not None:
                continue
            ogr = next((o for o in ogretmenler if o.id == sab.teacher_id), None)
            if not ogr:
                continue
            if izinli_mi(ogr.id, islem_tarihi) or engelli_mi(ogr.id, islem_tarihi):
                continue
            if ogr.id in gunluk_atanan:
                continue

            bosta_idler = [b for b in b_idler if b not in atanan_b]
            hedef = None
            if (
                algoritma == "2. Sabit Gün – Sabit Yer"
                and sab.location_id
                and sab.location_id in bosta_idler
            ):
            ...
            if hedef:
                nbt = DutySchedule(
                    school_id=school_id, date=islem_tarihi, duty_type=duty_type,
                    teacher_id=ogr.id, location_id=hedef, status="Asil",
                )
                db.add(nbt)
                db.commit()
                db.refresh(nbt)
                ...
                atanan_b.append(hedef)
                gunluk_atanan.append(ogr.id)
                haftalik_nobetler[ogr.id][week_num] = haftalik_nobetler[ogr.id].get(week_num, 0) + 1
                sabit_atanan += 1
                toplam_atanan += 1

    # ── FAZ 2: BİRİNCİ NÖBETLER (primary) ──────
    for g in range(1, bit.day + 1):
        islem_tarihi = date(yil, ay, g)
        hw = islem_tarihi.weekday()
        week_num = get_week_number(islem_tarihi)
        ...
        if not bosta_bolgeler:
            continue

        ogr_primary = []
        for o in ogretmenler:
            if o.id in gunluk_atanan:
                continue
            if izinli_mi(o.id, islem_tarihi) or engelli_mi(o.id, islem_tarihi):
                continue
            stat = STATUS_MAP.get(
                musaitlik.get(o.id, {}).get(hw, "🟢 1. Nöbet (Kesin İstiyorum)"), "primary"
            )
            if stat == "primary" and haftalik_nobetler[o.id].get(week_num, 0) == 0:
        ...
        for secilen in ogr_primary:
            ...
            nbt = DutySchedule(
                school_id=school_id, date=islem_tarihi, duty_type=duty_type,
                teacher_id=secilen.id, location_id=hedef_bolge.id, status="Asil",
            )
            db.add(nbt)
            db.commit()
            db.refresh(nbt)
    ...
    for g in range(1, bit.day + 1):
        ...
        for secilen in ogr_joker:
            ...
            nbt = DutySchedule(
                school_id=school_id, date=islem_tarihi, duty_type=duty_type,
                teacher_id=secilen.id, location_id=hedef_bolge.id, status="Joker",
            )
            db.add(nbt)
            db.commit()
            db.refresh(nbt)
    ...
    for g in range(1, bit.day + 1):
        ...
        for secilen in ogr_extra:
            ...
            nbt = DutySchedule(
                school_id=school_id, date=islem_tarihi, duty_type=duty_type,
                teacher_id=secilen.id, location_id=hedef_bolge.id, status="3. Nöbet",
            )
            db.add(nbt)
            db.commit()
            db.refresh(nbt)
    ...
    # ── FAZ 5: YEDEK ATAMA (sadece Excel için) ──
    for g in range(1, bit.day + 1):
        islem_tarihi = date(yil, ay, g)
        hw = islem_tarihi.weekday()
        if not gun_gecerli_mi(islem_tarihi, hw):
        ...
        musait_yedekler = [
            o for o in ogretmenler
            if o.id not in gunluk_atanan
            and not izinli_mi(o.id, islem_tarihi)
            and not engelli_mi(o.id, islem_tarihi)
        ]
    ...
    # Otomatik arşiv kaydı
    arsiv_label = f"Otomatik – {MONTHS_TR[ay - 1]} {yil}"
    db.query(BackupRecord).filter(
        BackupRecord.school_id == school_id, BackupRecord.label == arsiv_label
    ).delete()
    db.add(
        BackupRecord(
            school_id=school_id,
            ...
            payload_b64=base64.b64encode(yedek_olustur(db, school_id, yil, ay).encode()).decode(),
        )
    )
    db.commit()
    ...
    rapor = (
        f"#### 🧠 Dağıtım Özeti\n"
        f"* Nöbetler **{algoritma}** motoruyla dağıtıldı.\n"
        f"* **Toplam Atanan:** {toplam_atanan} &nbsp;|&nbsp; "
        f"**Joker:** {joker_atanan} &nbsp;|&nbsp; **Ekstra:** {ekstra_atanan}\n"
        f"* **Sabit Kural:** {sabit_atanan} nöbet sabit kurallara göre yerleştirildi."
    )
...
# PDF / HTML OLUŞTURUCU
# ─────────────────────────────────────────────
def pdf_olustur(okul, sec_yil: int, sec_ay: int, bolgeler_hi, pdf_rows: list, kural_listesi: list, hazirlayan: str) -> str:
    ...
    for row in pdf_rows:
        ...
        if row.get("Durum"):
            ...
            tbody += (
                f'<tr class="holiday" style="{week_sep}">'
                f'<td style="text-align:left;font-weight:700;white-space:nowrap;{week_sep}">{row["Tarih/Gün"]}</td>'
                f'<td colspan="{colspan}" style="{week_sep}">{row["Durum"]}</td>'
                f"</tr>"
            )
        else:
            ...
            tbody += (
                f'<tr style="{week_sep}">'
                f'<td style="text-align:left;font-weight:600;white-space:nowrap;{week_sep}">{row["Tarih/Gün"]}</td>'
                f"{tds}"
                f'<td style="font-weight:700;{week_sep}">{row.get("Müdür Yrd.", "")}</td>'
                f'<td style="{week_sep}"></td>'
                f"</tr>"
            )
...
.no-print{{
  position:sticky;top:0;z-index:999;background:#1d4ed8;
...
.rules-box{{border:1px solid #94a3b8;border-top:none;padding:4px 10px;font-size:calc(var(--fs) - 1pt);background:#f8fafc;}}
...
.sigs{{display:flex;justify-content:space-between;margin-top:10px;font-size:calc(var(--fs) - 0.5pt);text-align:center;}}
.sig-box .sig-line{{margin-top:25px;padding-top:2px;font-weight:700;width:220px;}}
.small-text{{font-size:8pt;color:#64748b;text-align:left;margin-top:5px;}}
</style>
</head>
<body>
<div class="no-print">
  <button class="btn-sz" onclick="changeSize(-1)">➖</button>
  <span id="sz-ind">Boyut: 9</span>
  <button class="btn-sz" onclick="changeSize(1)">➕</button>
  <button class="btn-pr" onclick="window.print()">🖨️ PDF Yazdır</button>
</div>
<div class="wrap">
  <div class="school-title">
    T.C.<br>{okul.name.upper()} MÜDÜRLÜĞÜ<br>
    {MONTHS_TR[sec_ay - 1].upper()} {sec_yil} ÖĞRETMEN NÖBET ÇİZELGESİ
  </div>
  <table>
    <thead>
      <tr>
        <th style="text-align:left;white-space:nowrap">TARİH / GÜN</th>
        {th_cols}
        <th>MD. YRD.</th>
        <th>İMZA</th>
      </tr>
  ...
  <div class="rules-box">
    <strong>📋 NÖBET YÖNERGESİ</strong>
    <ol>{kural_html}</ol>
  </div>
  <div class="sigs">
    <div class="sig-box">
      <div class="sig-line">HAZIRLAYAN<br>{hazirlayan}<br>Müdür Yardımcısı</div>
    </div>
    <div class="sig-box">
      <div class="sig-line">ONAYLAYAN<br>{date.today().strftime('%d.%m.%Y')}<br>{okul.manager_name}<br>Okul Müdürü</div>
    </div>
  </div>
  <div class="small-text">
    Not: <b>Koyu</b> isimler 2. nöbet (Joker), <b>*yıldızlı</b> isimler 3. nöbet (ekstra) tutanları gösterir.
  </div>
</div>
<script>
var cv = 9;
function changeSize(d) {{
  cv = Math.min(14, Math.max(5, parseFloat((cv + d * 0.1).toFixed(1))));
  document.getElementById('sz-ind').textContent = 'Boyut: ' + cv;
  document.documentElement.style.setProperty('--fs', cv + 'pt');
  var p = Math.max(1, Math.floor(cv / 2));
  document.documentElement.style.setProperty('--pad', p + 'px');
}}
</script>
</body>
</html>"""
...
def render_school_admin():
    ...
    if not kurallar:
        varsayilan = [
            "Nöbet görevine ders başlamadan 30 dakika önce başlanır.",
            "Öğrencilerin can güvenliğinden sorumludur.",
        ]
        for i, m in enumerate(varsayilan, 1):
            db.add(SchoolRule(school_id=okul.id, madde=m, sira=i))
        db.commit()
        kurallar = db.query(SchoolRule).filter(SchoolRule.school_id == okul.id).all()
    ...
    acik_tutanak = (
        ...
        .filter(IncidentLog.school_id == okul.id, IncidentLog.is_resolved == False)
        .count()
    )
    ...
    st.markdown(
        ...
        f"<div>"
        f"<h2>{okul.name} Yönetim Merkezi</h2>"
        f"<small>Okul Müdürü: <b>{okul.manager_name}</b> &nbsp;|&nbsp; "
        f"Profesyonel Nöbet Otomasyon Sistemi v9.1</small>"
        f"</div>"
        f"</div>",
    ...
    m4.markdown(metric_card("Açık Tutanak", acik_tutanak, "Çözüm bekliyor", "var(--danger)"), unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        sec_ay   = c2.selectbox("Ay", list(range(1, 13)), index=date.today().month - 1,
                                 format_func=lambda x: MONTHS_TR[x - 1], key="hi_ay")
        ...
        if col_clear.button("🗑️ Temizle", use_container_width=True, key="hi_temizle"):
            _temizle_ay_nobetleri(db, okul.id, sec_yil, sec_ay, "Ogretmen_Nobeti")
            st.session_state.pop("ai_rapor_hi", None)
            st.success("Hafta içi nöbetleri temizlendi.")
            st.rerun()
        ...
        program = (
            db.query(DutySchedule)
            .filter(
                DutySchedule.school_id == okul.id,
                extract("month", DutySchedule.date) == sec_ay,
                extract("year", DutySchedule.date) == sec_yil,
                DutySchedule.duty_type == "Ogretmen_Nobeti",
            )
            .all()
        )
        ...
        if program:
            ...
            oto_tatil   = turkiye_tatilleri(sec_yil)
            man_tatil   = {t.date: t.name for t in tatiller_all}
            hazirlayan  = st.selectbox(
                "Hazırlayan:",
                [m.name_surname for m in md_yard] if md_yard else ["Müdür Yardımcısı"],
                key="hi_mdyrd",
            )
            ...
            for g in range(1, calendar.monthrange(sec_yil, sec_ay)[1] + 1):
                ...
                if t.weekday() >= 5:
                    continue
                row_ex  = {"Tarih/Gün": fmt_date(t)}
                row_pdf = {"Tarih/Gün": fmt_date(t)}

                if t in oto_tatil or t in man_tatil:
                    tatil_adi = oto_tatil.get(t) or man_tatil.get(t)
                    row_ex["Durum"] = row_pdf["Durum"] = tatil_adi
                else:
                        ...
                        for p in program:
                            if p.date == t and p.location_id == b.id:
                                ...
                                if ydk:
                                    ...
                                    if yo and yo.name_surname not in yedekler_metni:
                                        yedekler_metni.append(yo.name_surname)

                        row_ex[b.name]  = kisi_ex
                        row_pdf[b.name] = kisi_pdf

                    row_ex["Yedek (Sadece Excel)"] = ", ".join(yedekler_metni) if yedekler_metni else "-"
                    row_ex["Müdür Yrd."] = row_pdf["Müdür Yrd."] = (
                        md_yard[md_idx % len(md_yard)].name_surname if md_yard else ""
                    )
                    md_idx += 1

                excel_rows.append(row_ex)
                pdf_rows.append(row_pdf)
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as w:
                ...
                for col in w.sheets["Nöbet"].columns:
                    w.sheets["Nöbet"].column_dimensions[col[0].column_letter].width = 25
            st.download_button(
                "📊 Excel İndir (Yedekler Excel'dedir)",
                data=buf.getvalue(),
                file_name=f"Nobet_{MONTHS_TR[sec_ay - 1]}_{sec_yil}.xlsx",
            ...
            st.markdown(
                '<div class="warn-box">⚠️ Bu ay için hafta içi nöbet programı oluşturulmamış. '
                "Yukarıdaki <b>Otomatik Dağıt</b> butonunu kullanın.</div>",
                unsafe_allow_html=True,
            )
    ...
    with tabs[1]:
        ...
        hs_ay   = hs2.selectbox("Ay", list(range(1, 13)), index=date.today().month - 1,
                                 format_func=lambda x: MONTHS_TR[x - 1], key="hs_ay")
        ...
        if c_cl.button("🗑️ Temizle", use_container_width=True, key="hs_temizle"):
            _temizle_ay_nobetleri(db, okul.id, hs_yil, hs_ay, "Haftasonu")
            st.session_state.pop("ai_rapor_hs", None)
            st.success("Hafta sonu nöbetleri temizlendi.")
            st.rerun()
        ...
        hs_prog = (
            ...
            .filter(
                DutySchedule.school_id == okul.id,
                extract("month", DutySchedule.date) == hs_ay,
                extract("year", DutySchedule.date) == hs_yil,
                DutySchedule.duty_type == "Haftasonu",
            )
            .all()
        )
        ...
        if hs_prog:
            oto_tatil2 = turkiye_tatilleri(hs_yil)
            man_tatil2 = {t.date: t.name for t in tatiller_all}
            rows_hs    = []
            for g in range(1, calendar.monthrange(hs_yil, hs_ay)[1] + 1):
                t = date(hs_yil, hs_ay, g)
                if t.weekday() < 5 or t in oto_tatil2 or t in man_tatil2:
                    continue
            ...
            if rows_hs:
                ...
                with pd.ExcelWriter(buf2, engine="openpyxl") as w:
                    pd.DataFrame(rows_hs).to_excel(w, index=False, sheet_name="HafSonu")
                st.download_button(
                    "📊 Hafta Sonu Excel İndir",
                    data=buf2.getvalue(),
                    file_name=f"HafSonu_{MONTHS_TR[hs_ay - 1]}_{hs_yil}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
        else:
            st.markdown(
                '<div class="warn-box">⚠️ Bu ay için hafta sonu nöbet programı oluşturulmamış.</div>',
                unsafe_allow_html=True,
            )

    # ──────────────────────────────────────────
    # TAB 2 — MÜSAİTLİK MATRİSİ
    # ──────────────────────────────────────────
    with tabs[2]:
        st.markdown('<div class="section-title">7 Günlük İnteraktif Müsaitlik Matrisi</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="info-box">'
            "🟢 <b>1. Nöbet (Talep):</b> Temel öncelik. &nbsp;&nbsp;"
            "🟡 <b>2. Nöbet (Joker):</b> Boşlukta 2. nöbet yazılabilir. &nbsp;&nbsp;"
            "🔵 <b>3. Nöbet (Ekstra):</b> Kritik açıkta 3. nöbet. &nbsp;&nbsp;"
            "🔴 <b>Müsait Değil:</b> Kesinlikle nöbet verilmez."
            "</div>",
            unsafe_allow_html=True,
        )
        ...
        for o in ogretmenler:
            ...
            for i, d_name in enumerate(DAYS_TR):
                ...
                row_dict[d_name] = REV_STATUS_MAP.get(raw_stat, STATUS_OPTS[0])
            matrix_rows.append(row_dict)

        if matrix_rows:
            col_cfg = {
                "ID": st.column_config.NumberColumn("ID", disabled=True),
                "Öğretmen Adı Soyadı": st.column_config.TextColumn("Öğretmen", disabled=True),
            }
            for d_name in DAYS_TR:
                col_cfg[d_name] = st.column_config.SelectboxColumn(d_name, options=STATUS_OPTS)
            ...
            if st.button("💾 Tüm Matrisi Kaydet", type="primary", use_container_width=True):
                for _, r in edited_matrix.iterrows():
                    t_id = int(r["ID"])
                    for i, d_name in enumerate(DAYS_TR):
                        set_musaitlik(db, okul.id, t_id, i, STATUS_MAP[r[d_name]])
                st.success("✅ Müsaitlik öncelikleri güncellendi!")
                st.rerun()

        st.divider()
        st.markdown('<div class="section-title">Hafta İçi / Hafta Sonu Havuz Tercihleri</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="info-box">Öğretmenlerin hangi havuza dahil olacağını işaretleyin. '
            "Otomatik dağıtımda yalnızca işaretli havuzdaki öğretmenler kullanılır.</div>",
            unsafe_allow_html=True,
        )
        ...
        if ogr_veri:
            ...
            if st.button("💾 Havuz Tercihlerini Kaydet", type="primary", key="havuz_kaydet"):
                for _, r in ed_df.iterrows():
                    db.query(TeacherDutySetting).filter(
                        TeacherDutySetting.teacher_id == int(r["ID"])
                    ).update({
                        "hafta_ici_tutar": bool(r["Hafta İçi"]),
                        "hafta_sonu_tutar": bool(r["Hafta Sonu"]),
                    })
                db.commit()
                st.success("✅ Havuz tercihleri kaydedildi!")
                st.rerun()

    # ──────────────────────────────────────────
    # TAB 3 — İZİN & SABİT KURALLAR
    # ──────────────────────────────────────────
    with tabs[3]:
        c_mat, c_sab = st.columns(2)

        with c_mat:
            st.markdown('<div class="section-title">Tarih Aralığı Kapatma</div>', unsafe_allow_html=True)
            st.markdown(
                '<div class="info-box">Raporlu veya izinli öğretmen için tarih aralığını nöbete kapatın.</div>',
                unsafe_allow_html=True,
            )
            with st.form("tarih_kapat_form"):
                ...
                range_bas = st.date_input("Başlangıç Tarihi")
                range_bit = st.date_input("Bitiş Tarihi")
                if st.form_submit_button("🔒 Aralığı Engelle", type="primary"):
                    if range_bas <= range_bit:
                        ...
                        db.commit()
                        st.success("✅ Tarih aralığı nöbete kapatıldı!")
                        st.rerun()
                    else:
            ...
            if kap:
                kap_data = []
                for dp in kap:
                    ogr_obj = db.query(User).get(dp.teacher_id)
                    if ogr_obj:
                        kap_data.append({
                            "#": dp.id,
                            "Öğretmen": ogr_obj.name_surname,
                            "Engellenen Gün": dp.blocked_date.strftime("%d.%m.%Y"),
                        })
                st.dataframe(pd.DataFrame(kap_data), hide_index=True, use_container_width=True)
                del_id = st.number_input("Silmek istediğiniz # (ID)", min_value=0, step=1, key="del_engel_id")
                if st.button("🗑️ Engeli Kaldır", key="del_engel_btn") and del_id:
                    db.query(DatePreference).filter(DatePreference.id == int(del_id)).delete()
                    db.commit()
                    st.rerun()

        with c_sab:
            st.markdown('<div class="section-title">Öğretmen Gün Sabitleme</div>', unsafe_allow_html=True)
            st.markdown(
                '<div class="info-box">Öğretmene sabit gün atanır; lokasyon otomatik döngüsel değişir.</div>',
                unsafe_allow_html=True,
            )
            with st.form("sabit_form"):
                ...
                s_gun = st.selectbox(
                    "Sabitlenecek Gün",
                    [("Seçilmedi", -1)] + [(g, i) for i, g in enumerate(DAYS_TR[:5])],
                    format_func=lambda x: x[0],
                )
                if st.form_submit_button("📌 Kuralı Sabitle", type="primary"):
                    db.query(FixedRule).filter(FixedRule.teacher_id == s_ogr[0]).delete()
                    if s_gun[1] != -1:
                        db.add(FixedRule(school_id=okul.id, teacher_id=s_ogr[0], day_of_week=s_gun[1], location_id=None))
                    db.commit()
            ...
            if sab_list:
                sab_data = []
                for s in sab_list:
                    ogr_obj = db.query(User).get(s.teacher_id)
                    if ogr_obj:
                        sab_data.append({
                            "Öğretmen": ogr_obj.name_surname,
                            "Sabit Gün": DAYS_TR[s.day_of_week] if s.day_of_week is not None else "Her Gün",
                            "Bölge": "Döngüsel Rotasyon",
                        })
                st.dataframe(pd.DataFrame(sab_data), hide_index=True, use_container_width=True)

    # ──────────────────────────────────────────
    # TAB 4 — NÖBET DEĞİŞİMİ
    # ──────────────────────────────────────────
    with tabs[4]:
        st.markdown('<div class="section-title">🔄 Nöbet Değişimi ve Not Ekleme</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="info-box">Bir öğretmen nöbete gelemediğinde yerine bakanı buradan güncelleyin. '
            "Sayaçlar otomatik düzeltilir.</div>",
            unsafe_allow_html=True,
        )

        deg_tarih = st.date_input("Nöbet Tarihi Seçin", key="degisim_tarih")
        gunluk_nobetler = (
            db.query(DutySchedule)
            .filter(DutySchedule.school_id == okul.id, DutySchedule.date == deg_tarih)
            .all()
        )

        if gunluk_nobetler:
            def nobet_etiketi(nbt_id: int) -> str:
                n = next((n for n in gunluk_nobetler if n.id == nbt_id), None)
                if not n:
                    return str(nbt_id)
                loc = db.query(Location).get(n.location_id)
                ogr = db.query(User).get(n.teacher_id)
                return f"{loc.name if loc else '?'} — {ogr.name_surname if ogr else '?'}"

            with st.form("nobet_degisim"):
                secili_nobet_id = st.selectbox(
                    "Değiştirilecek Nöbet",
                    [n.id for n in gunluk_nobetler],
                    format_func=nobet_etiketi,
                )
                yeni_ogretmen = st.selectbox(
                    "Yerine Nöbet Tutacak Öğretmen",
                    [(o.id, o.name_surname) for o in ogretmenler],
                ...
                if st.form_submit_button("🔄 Nöbeti Devret", type="primary"):
                    ...
                    n.teacher_id = yeni_ogr.id if yeni_ogr else n.teacher_id
                    n.status     = f"Değişim ({not_metni})" if not_metni else "Değişim"
                    db.commit()
                    st.success("✅ Nöbet başarıyla devredildi, sayaçlar güncellendi!")
                    st.rerun()
        else:
            st.markdown(
                '<div class="warn-box">⚠️ Seçilen tarihte planlanmış nöbet bulunmuyor.</div>',
                unsafe_allow_html=True,
            )
    ...
    with tabs[5]:
        ...
        with st.form("kural_ekle_form"):
            ...
            if st.form_submit_button("➕ Madde Ekle", type="primary") and k_madde.strip():
                db.add(SchoolRule(school_id=okul.id, madde=k_madde.strip(), sira=int(k_sira)))
                db.commit()
                st.rerun()

        for k in sorted(kurallar, key=lambda x: x.sira or 0):
            with st.expander(f"Madde {k.sira}: {k.madde[:60]}{'…' if len(k.madde) > 60 else ''}"):
                ...
                if bc1.button("💾 Güncelle", key=f"uk_{k.id}"):
                    k.madde = nt
                    k.sira  = ns
                    db.commit()
                    st.rerun()
                if bc2.button("🗑️ Sil", key=f"dk_{k.id}"):
                    db.delete(k)
                    db.commit()
                    st.rerun()
    ...
    with tabs[6]:
        ...
        c_man, c_toplu, c_idare = st.tabs(["✍️ Manuel Yönetim", "📂 Excel Toplu Yükleme", "👔 İdareci Yönetimi"])
        ...
        with c_man:
            with st.form("ogr_ekle"):
                ...
                osifre = f2.text_input("Şifre", value="1234")
                if st.form_submit_button("➕ Yeni Öğretmen Ekle", type="primary") and oad.strip() and otc.strip():
                    mevcut = db.query(User).filter(User.username == otc.strip()).first()
                    if mevcut:
                        st.error("❌ Bu TC / Kullanıcı Adı zaten kayıtlı!")
                    else:
                        yeni_ogr = User(
                            school_id=okul.id,
                            role="ogretmen",
                            username=otc.strip(),
                            email=f"{otc.strip()}@meb",
                            password_hash=sifre_olustur(osifre),
                            plain_password=osifre,
                            name_surname=oad.strip(),
                            branch=obrans.strip(),
                            is_approved=True,
                            status="Aktif",
                        ...
                        try:
                            ...
                            db.rollback()
                            st.error("❌ Veritabanı hatası. Kayıt eklenemedi.")
                        st.rerun()

            st.write("---")
            if st.button("🗑️ Pasif Olanları Toplu Sil", key="pasif_sil"):
                db.query(User).filter(
                    User.school_id == okul.id, User.role == "ogretmen", User.status == "Pasif"
                ).delete()
                db.commit()
                st.rerun()
                ...
                with st.expander(f"{badge} {o.name_surname} — {o.branch or 'Branşsız'}"):
                    with st.form(f"edit_{o.id}"):
                        ...
                        if st.form_submit_button("💾 Bilgileri Güncelle"):
                            ...
                            o.username      = e_usr
                            o.branch        = e_br
                            o.plain_password = e_pass
                            o.password_hash = sifre_olustur(e_pass)
                            db.commit()
                            st.success("Güncellendi!")
                            st.rerun()
                    b1, b2 = st.columns(2)
                    if b1.button("🔄 Pasif/Aktif Yap", key=f"tog_{o.id}"):
                        o.status = "Pasif" if durum == "Aktif" else "Aktif"
                        db.commit()
                        st.rerun()
                    if b2.button("❌ Tamamen Sil", key=f"del_{o.id}"):
                        db.delete(o)
                        db.commit()
                        st.rerun()
        ...
        with c_toplu:
            ...
            st.download_button(
                ...
                file_name="Ogretmen_Ekleme_Sablonu.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )

            uploaded_file = st.file_uploader("📤 Doldurulmuş Şablonu Yükle", type=["xlsx", "xls"], key="excel_uploader_ogr")
            if uploaded_file and st.button("🚀 Toplu Ekle", type="primary", key="btn_excel_toplu_ekle"):
                ...
                for _, row in df.iterrows():
                    ...
                    if not ogr_tc or ogr_tc == "nan" or not ogr_ad or ogr_ad == "nan":
                        atlanan += 1
                        continue
                    ...
                    try:
                        ...
                        db.rollback()
                        atlanan += 1
                st.success(f"✅ {eklenen} öğretmen eklendi. {atlanan} kayıt atlandı (mevcut veya hatalı).")
                st.rerun()

        with c_idare:
            st.markdown("**Okul Müdürü**")
            with st.form("md_mudur_form"):
                yeni_mudur = st.text_input("Okul Müdürü İsmi", value=okul.manager_name)
                if st.form_submit_button("💾 Müdürü Güncelle", type="primary"):
                    okul.manager_name = yeni_mudur.strip()
                    db.commit()
                    st.success("Müdür ismi güncellendi!")
                    st.rerun()

            st.write("---")
            st.markdown("**Müdür Yardımcıları**")
            with st.form("md_ekle_form"):
                md_ad = st.text_input("Yeni Müdür Yardımcısı")
                if st.form_submit_button("➕ Ekle", type="primary") and md_ad.strip():
                    db.add(AssistantPrincipal(school_id=okul.id, name_surname=md_ad.strip()))
                    db.commit()
                    st.rerun()

            for m in md_yard:
                with st.expander(f"👔 {m.name_surname}"):
                    e_md = st.text_input("İsim Güncelle", value=m.name_surname, key=f"emd_{m.id}")
                    bc1, bc2 = st.columns(2)
                    if bc1.button("💾 Kaydet", key=f"updmd_{m.id}"):
                        m.name_surname = e_md.strip()
                        db.commit()
                        st.success("Güncellendi!")
                        st.rerun()
                    if bc2.button("🗑️ Sil", key=f"delmd_{m.id}"):
                        db.delete(m)
                        db.commit()
                        st.rerun()

    # ──────────────────────────────────────────
    # TAB 7 — BÖLGELER & TATİLLER
    # ──────────────────────────────────────────
    with tabs[7]:
        ...
        with c_b:
            ...
            with st.form("bolge_ekle"):
                b_ad  = st.text_input("Bölge Adı")
                b_tip = st.selectbox("Tür", ["Hafta İçi", "Hafta Sonu"])
                if st.form_submit_button("➕ Ekle", type="primary") and b_ad.strip():
                    db.add(Location(school_id=okul.id, name=b_ad.strip(), location_type=b_tip))
                    db.commit()
                    st.rerun()

            for b in bolgeler_all:
                icon = "🔵" if b.location_type != "Hafta Sonu" else "🟠"
                bc1, bc2 = st.columns([5, 1])
                bc1.write(f"{icon} {b.name}  —  {b.location_type}")
                if bc2.button("Sil", key=f"db_{b.id}"):
                    db.delete(b)
                    db.commit()
                    st.rerun()
        ...
        with c_t:
            ...
            with st.form("tatil_ekle"):
                t_ad  = st.text_input("Tatil Adı / Neden")
                t_tar = st.date_input("Tarih")
                if st.form_submit_button("🏖️ Ekle", type="primary") and t_ad.strip():
                    db.add(HolidayManager(school_id=okul.id, name=t_ad.strip(), date=t_tar))
                    db.commit()
                    st.rerun()
            ...
            for t in tatiller_all:
                ...
                if tc2.button("Sil", key=f"dt_{t.id}"):
                    db.delete(t)
                    db.commit()
                    st.rerun()
    ...
    with tabs[8]:
        ...
        olaylar = (
            db.query(IncidentLog)
            .filter(IncidentLog.school_id == okul.id)
            .order_by(IncidentLog.date.desc())
            .all()
        )
        for olay in olaylar:
            t_isim = next((o.name_surname for o in ogretmenler if o.id == olay.teacher_id), "Bilinmiyor")
            durum_str = "🟢 Çözüldü" if olay.is_resolved else "🔴 Açık"
            with st.expander(f"[{olay.date.strftime('%d.%m.%Y')}] {olay.incident_type} — {t_isim}  |  {durum_str}"):
                st.write(olay.description)
                if not olay.is_resolved:
                    if st.button("✅ Çözüldü Olarak İşaretle", key=f"res_{olay.id}"):
                        olay.is_resolved = True
                        db.commit()
                        st.rerun()
    ...
    with tabs[9]:
        ...
        sec_ist_ay  = s2.selectbox(
            "Dönem",
            list(range(0, 13)),
            format_func=lambda x: "Tüm Yıl" if x == 0 else MONTHS_TR[x - 1],
            key="ist_ay",
        )
        if s3.button("🔄 Aylık Sayaçları Sıfırla", key="sayac_sifirla"):
            for o in ogretmenler:
                o.monthly_duty_count = 0
            db.commit()
            st.rerun()

        ist_data = []
        for o in ogretmenler:
            q = db.query(DutySchedule).filter(
                DutySchedule.school_id == okul.id,
                DutySchedule.teacher_id == o.id,
                extract("year", DutySchedule.date) == sec_ist_yil,
            )
            if sec_ist_ay:
                q = q.filter(extract("month", DutySchedule.date) == sec_ist_ay)
            toplam = q.count()
            hs     = q.filter(DutySchedule.duty_type == "Haftasonu").count()
            ist_data.append({
                "Öğretmen": o.name_surname,
                "Toplam": toplam,
                "Hafta İçi": toplam - hs,
                "Hafta Sonu": hs,
                "Ay Sayacı": o.monthly_duty_count or 0,
                "Yıl Sayacı": o.yearly_duty_count or 0,
            })

        if ist_data:
            st.dataframe(
                pd.DataFrame(ist_data).sort_values("Toplam", ascending=False),
                hide_index=True,
                use_container_width=True,
            )
        ...
        with st.form("sayac_duzenle"):
            ...
            ed_ay   = ec1.number_input("Aylık Sayaç", value=0, min_value=0)
            ed_yil  = ec2.number_input("Yıllık Sayaç", value=0, min_value=0)
            if st.form_submit_button("💾 Sayacı Güncelle", type="primary"):
                ogr_ed = db.query(User).get(ed_ogr[0])
                if ogr_ed:
                    ogr_ed.monthly_duty_count = int(ed_ay)
                    ogr_ed.yearly_duty_count  = int(ed_yil)
                    db.commit()
                st.rerun()
    ...
    with tabs[10]:
        ...
        with arc_col1:
            ...
            with st.form("yedek_olustur_form"):
                y_yil    = st.number_input("Yıl", value=date.today().year, min_value=2020, key="arc_yil")
                y_ay     = st.selectbox("Ay", list(range(1, 13)), index=date.today().month - 1,
                                        format_func=lambda x: MONTHS_TR[x - 1], key="arc_ay")
                y_etiket = st.text_input("Etiket", value="Manuel Yedek")
                if st.form_submit_button("💾 Yedek Oluştur", type="primary"):
                    db.add(BackupRecord(
                        school_id=okul.id,
                        label=f"{y_etiket} – {MONTHS_TR[y_ay - 1]} {y_yil}",
                        created_at=str(datetime.now()),
                        payload_b64=base64.b64encode(
                            yedek_olustur(db, okul.id, int(y_yil), y_ay).encode()
                        ).decode(),
                    ))
                    db.commit()
                    st.rerun()
        ...
        with arc_col2:
            ...
            yukl = st.file_uploader("JSON Dosyası Yükle", type=["json"], key="json_yukle")
            if yukl:
                try:
                    payload = json.loads(yukl.read())
                    if st.button("🔄 Geri Yükle", type="primary", key="geri_yukle_btn"):
                        for n in payload.get("nobetler", []):
                            mevcut = db.query(DutySchedule).filter(
                                DutySchedule.school_id == okul.id,
                                DutySchedule.date == date.fromisoformat(n["date"]),
                                DutySchedule.teacher_id == n["teacher_id"],
                                DutySchedule.location_id == n["location_id"],
                            ).first()
                            if not mevcut:
                                db.add(DutySchedule(
                                    school_id=okul.id,
                                    date=date.fromisoformat(n["date"]),
                                    duty_type=n["duty_type"],
                                    teacher_id=n["teacher_id"],
                                    location_id=n["location_id"],
                                    status=n.get("status", "Planlandi"),
                                ))
                        db.commit()
                        st.success("✅ Arşiv başarıyla geri yüklendi!")
                        st.rerun()
        ...
        kayitli = (
            ...
            .filter(BackupRecord.school_id == okul.id)
        ...
        if not kayitli:
            st.markdown(
                '<div class="warn-box">Henüz arşiv kaydı yok. '
                "Otomatik dağıtım yaptığınızda buraya otomatik kaydedilir.</div>",
                unsafe_allow_html=True,
            )
        else:
            yillar = sorted(
                {br.created_at[:4] for br in kayitli if br.created_at},
                reverse=True,
            )
            sec_yil_arsiv = st.selectbox("Yıla Göre Filtrele", ["Tümü"] + yillar, key="arsiv_yil")

            for br in kayitli:
                if sec_yil_arsiv != "Tümü" and not br.created_at.startswith(sec_yil_arsiv):
                    continue
                tarih_str = br.created_at[:16] if br.created_at else ""
                st.markdown(
                    f'<div class="arsiv-card">'
                    f'<div class="arsiv-badge">📦<br><span style="font-size:.75rem">{tarih_str[:10]}</span></div>'
                    f"<div style='flex:1'>"
                    f"<div style='font-weight:700;color:#0f1f3d'>{br.label}</div>"
                    f"<div style='font-size:.78rem;color:#64748b'>{tarih_str}</div>"
                    f"</div>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
                ...
                if br.payload_b64:
                    ...
                    exp_col1.download_button(
                        "📥 JSON İndir",
                        data=json_bytes,
                        file_name=f"arsiv_{br.id}.json",
                        mime="application/json",
                        key=f"json_dl_{br.id}",
                    )
                    try:
                        ...
                        if nobetler_data:
                            df_arsiv = pd.DataFrame([
                                {
                                    "Tarih": n["date"],
                                    "Tür": n["type"],
                                    "Öğretmen": ogr_map.get(n["teacher_id"], "?"),
                                    "Bölge": bolge_map.get(n["location_id"], "?"),
                                    "Durum": n.get("status", ""),
                                }
                            ...
                            with pd.ExcelWriter(buf_arsiv, engine="openpyxl") as w:
                                df_arsiv.to_excel(w, index=False, sheet_name="Arşiv")
                            exp_col2.download_button(
                                "📊 Excel İndir",
                                data=buf_arsiv.getvalue(),
                                file_name=f"arsiv_{br.id}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                key=f"exc_dl_{br.id}",
                            )
                    except Exception:
                ...
                if exp_col3.button("🗑️ Sil", key=f"del_br_{br.id}"):
                    db.delete(br)
                    db.commit()
                    st.rerun()
